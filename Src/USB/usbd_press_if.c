...
/*============ Exported Variables ============*/
bool boPressReportToSend = 0;
uint8_t usb_hid_press_report_in[USBD_PRESS_HID_REPORT_IN_SIZE] = {0}; // buffer used to send report to host
uint8_t *pTBPCommandReportPress = 0;


/*============ Local Function Prototypes ============*/
static int8_t PRESS_HID_Init_FS(void);
static int8_t PRESS_HID_DeInit_FS(void);
static int8_t PRESS_HID_OutEvent_FS(uint8_t* state);


 /* USB Press HID Report Descriptor */
__ALIGN_BEGIN static uint8_t PRESS_HID_ReportDesc_FS[USBD_PRESS_HID_REPORT_DESC_SIZE] __ALIGN_END =
{
                     // item code|no. bytes
        ...
        0x95, USBD_PRESS_HID_REPORT_IN_SIZE,   // 94|2 ReportCount(size) = repeat count of previous item, 64 byte IN report
        ...
        0x26, 0xFF, 0x00,   // 24|1   , Logical Maximum(255 for signed byte?)
        0x75, 0x08,         // 74|1   , Report Size(8) = field size in bits = 1 byte
        0x95, USBD_PRESS_HID_REPORT_OUT_SIZE,   // 94|2 ReportCount(size) = repeat count of previous item, 64 byte OUT report
        ...
        0x26, 0xFF, 0x00,   // 24|1   , Logical Maximum(255 for signed byte)
        0x75, 0x08,         // 74|1   , Report Size(8) = field size in bits = 1 byte
        0x95, USBD_PRESS_HID_FEATURE_SIZE,         // 94|1   , ReportCount in byte
...
USBD_PRESS_HID_ItfTypeDef USBD_PressHID_fops_FS =
{
  ...
  PRESS_HID_Init_FS,
  PRESS_HID_DeInit_FS,
  PRESS_HID_OutEvent_FS
};

/*============ Functions ============*/
/**
  * @brief  Initializes the PRESS HID media low layer
...
/**
  * @brief  DeInitializes the PRESS HID media low layer
  * @retval USBD_OK if all operations are OK else USBD_FAIL
  */
static int8_t PRESS_HID_DeInit_FS(void)
{
...
/**
  * @brief  Manage the PRESS HID class events
...
static int8_t PRESS_HID_OutEvent_FS(uint8_t* state)
{
    ...
    target_interface = PRESS_INTERFACE_NUM;

    GPIOC->ODR ^= GPIO_ODR_9;

    // set pointer to out buffer
    pTBPCommandReportPress = state;
    return (USBD_OK);
}
